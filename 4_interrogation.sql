...
-- Patients féminins inscrits dans des essais démarrant entre deux dates (BETWEEN)
...
-- Employés contenant 'Chef' dans la fonction (masque LIKE) sans doublons
SELECT DISTINCT nom_emp, prenom_emp, fonction_emp, service_emp
FROM Employe
WHERE fonction_emp LIKE '%Chef%'
ORDER BY service_emp, nom_emp;

-- Médicaments de marques données (IN) triés par date_AMM décroissante
...
-- Nombre de patients par essai (HAVING >= 5)
...
-- Nombre de brevets par molécule (garder > 1)
...
-- Projets sans chercheurs affectés (NOT EXISTS)
...
-- Employés qui ne collaborent avec aucun partenaire (NOT IN)
...
-- Essais avec au moins un patient mineur (EXISTS corrélé)
SELECT e.code_essai, e.phase_clinique, e.date_debut, e.date_fin
FROM essai_clinique e
WHERE EXISTS (
  SELECT 1 FROM patient p
  WHERE p.code_essai = e.code_essai
    AND (EXTRACT(year FROM AGE(p.date_naissance)) < 18)
);

-- Médicaments dont la date_AMM est supérieure à toutes les dates_AMM d'autres marques (ALL)
SELECT m.num_AMM, m.nom_marque, m.date_AMM
FROM medicament m
...
-- Chercheurs travaillant sur un projet ayant un budget_alloué supérieur à la moyenne (ANY)
